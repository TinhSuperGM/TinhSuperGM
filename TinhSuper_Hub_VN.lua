...
Countdown = L_1_[93]["Main"]:AddParagraph({
	["Title"] = "Thời gian nhận quà Giáng Sinh",
	["Content"] = ""
})
...
L_1_[127] = function()
	local now = os.time()
	local passed = now % L_1_[79]
	return L_1_[79] - passed
end

L_1_[135] = function()
	local timeLeft = L_1_[127]()

	if timeLeft <= 0 then
		Countdown:SetDesc("Sẵn sàng nhận quà!")
		return
	end
...
spawn(function()
	while wait(Sec) do
		if _G["AutoFarmCandy"] then
			pcall(function()
				local Character = L_1_[136].Character or L_1_[136].CharacterAdded:Wait()
				local HRP = Character:WaitForChild("HumanoidRootPart")
				local Level = L_1_[136].Data.Level.Value
				local IsFar = L_1_[124]()

				if Level >= 2600 and (not IsFar and not L_1_[107] and not L_1_[109]) then
					L_1_[107] = true
					...
					until not _G["AutoFarmCandy"]
						or (HRP.Position - TargetCF.Position).Magnitude <= 8
						or tries > 20
					...
					repeat
						task.wait(0.5)
						if L_1_[124]() or (HRP.Position - TargetCF.Position).Magnitude > 50 then
							break
						end
					until not _G["AutoFarmCandy"] or tick() - start > 15
					...
					if closest then
						...
						until not _G["AutoFarmCandy"]
							...
							or (HRP.Position - closest.HumanoidRootPart.Position).Magnitude > 1100
					else
...
L_1_[58] = CFrame.new(521.277832, 9.30464935, -3339.21753, 0.563576221, 0, 0.82606405, 0, 1, 0, -0.82606405, 0, 0.563576221)
L_1_[28] = CFrame.new(-5405.59082, 8.06555939, 1451.85144, -0.987686276, 0, 0.156449571, 0, 1, 0, -0.156449571, 0, -0.987686276)
...
local GiftMapVNtoKey = {
	["Đảo ở sea 1"] = "GiftMap1",
	["Đảo ở sea 2"] = "GiftMap2",
	["Đảo ở sea 3"] = "GiftMap3"
}
...
L_1_[93]["Main"]:AddDropdown({
	["Name"] = "Chọn đảo quà",
	["Options"] = GiftMapVNList,
	["Callback"] = function(Value)
		_G["SelectedGiftMap"] = GiftMapVNtoKey[Value]
	end
})

L_1_[93]["Main"]:AddToggle({
	["Name"] = "Dịch chuyển đến đảo quà",
	["Default"] = false,
	["Callback"] = function(Value)
		_G["AutoTP_Gift"] = Value
	end
})
task.spawn(function()
	while task.wait(1) do
		if _G["AutoTP_Gift"] and _G["SelectedGiftMap"] then
			local cf = GiftMapCFrame[_G["SelectedGiftMap"]]
			if cf then
				_tp(cf)
			end
		end
...
L_1_[93]["Main"]:AddToggle({
	["Name"] = "Tự động dịch chuyển & nhặt quà",
	["Description"] = "Yêu cầu chọn đảo",
	["Default"] = false,
	["Callback"] = function(Value)
		_G["AutoTPAndCollect"] = Value
	end
})
task.spawn(function()
	...
	while task.wait(0.3) do
		...
		local present = world:FindFirstChild("Present")
		local box = present and present:FindFirstChild("Box")

		if box then
			isTeleported = false
			_tp(box.CFrame * CFrame.new(0, 3, 0))
			task.wait(0.2)
			for i = 1, 3 do
		...
		elseif not isTeleported and _G["SelectedGiftMap"] then
			local cf = GiftMapCFrame[_G["SelectedGiftMap"]]
			if cf then
				_tp(cf)
				isTeleported = true
...
L_1_[93]["Main"]:AddToggle({
	["Name"] = "Tự động quay Gacha mùa đông",
	["Description"] = "Tốn 100 kẹo",
	["Default"] = false,
	["Callback"] = function(Value)
		_G["AutoGachaCandy"] = Value
	end
})
...
local HolidayGiftMap = {
	["Quà Giáng Sinh Thường"]     = "Uncommon Holiday Gift",
	["Quà Giáng Sinh Hiếm"]       = "Rare Holiday Gift",
	["Quà Giáng Sinh Huyền Thoại"]= "Legendary Holiday Gift",
	["Quà Giáng Sinh Thần Thoại"] = "Mythical Holiday Gift"
}

L_1_[93]["Main"]:AddDropdown({
	["Name"] = "Chọn quà Giáng Sinh",
	["Options"] = {
		"Quà Giáng Sinh Thường",
		"Quà Giáng Sinh Hiếm",
		"Quà Giáng Sinh Huyền Thoại",
		"Quà Giáng Sinh Thần Thoại"
	},
	["Default"] = "",
	["Callback"] = function(Value)
		-- Lưu giá trị TIẾNG ANH để dùng cho server
		_G["SelectedGift"] = HolidayGiftMap[Value]
	end
})

L_1_[93]["Main"]:AddToggle({
	["Name"] = "Tự động lấy quà từ kho",
	["Default"] = false,
	["Callback"] = function(Value)
		_G["AutoUnStoreGift"] = Value
	end
})
...
AutoStoreGift = L_1_[93]["Main"]:AddToggle({
	["Name"] = "Tự động cất quà",
	["Description"] = "",
	["Default"] = false,
	["Callback"] = function(Value)
		_G["AutoStoreGift"] = Value
	end
})
...
L_1_[93]["Main"]:AddButton({
	["Name"] = "Mua 200 Fragment",
	["Description"] = "Tốn 50 Kẹo",
	["Callback"] = function()
		pcall(function()
			game:GetService("ReplicatedStorage")
				.Remotes
				.CommF_
				:InvokeServer("Candies", "Buy", 2, 1)
		end)
	end
})

L_1_[93]["Main"]:AddButton({
	["Name"] = "Mua 500 Fragment",
	["Description"] = "Tốn 100 Kẹo",
	["Callback"] = function()
		pcall(function()
			game:GetService("ReplicatedStorage")
...
L_1_[93]["Main"]:AddButton({
	["Name"] = "Mua x2 EXP (15 phút)",
	["Description"] = "Tốn 75 Kẹo",
	["Callback"] = function()
		pcall(function()
			game:GetService("ReplicatedStorage")
				.Remotes
...
L_1_[93]["Main"]:AddButton({
	["Name"] = "Mua mũ Yêu Tinh (Elf Hat)",
	["Description"] = "Tốn 250 Kẹo",
	["Callback"] = function()
		pcall(function()
			game:GetService("ReplicatedStorage")
				.Remotes
				.CommF_
				:InvokeServer("Candies", "Buy", 3, 1)
		end)
	end
})

L_1_[93]["Main"]:AddButton({
	["Name"] = "Mua mũ Ông Già Noel (Santa Hat)",
	["Description"] = "Tốn 500 Kẹo",
	["Callback"] = function()
		pcall(function()
			game:GetService("ReplicatedStorage")
				.Remotes
				.CommF_
				:InvokeServer("Candies", "Buy", 3, 2)
		end)
	end
})

L_1_[93]["Main"]:AddButton({
	["Name"] = "Mua xe trượt tuyết (Sleigh)",
	["Description"] = "Tốn 1000 Kẹo",
	["Callback"] = function()
		pcall(function()
			game:GetService("ReplicatedStorage")
				.Remotes
				.CommF_
				:InvokeServer("Candies", "Buy", 3, 3)
		end)
	end
})
L_1_[93]["Main"]:AddSection({
			...
			pcall(function()
				...
				for _, chest in pairs(chests) do
					...
					if (not SelectedIsland or chest:IsDescendantOf(SelectedIsland)) then
						if not chest:GetAttribute("IsDisabled") and dist < closestDist then
							closestDist = dist
							closestChest = chest
						end
					end
	...
	["Callback"] = function(Value)
		...
		if Value then
			...
			local function getChests()
				...
				local valid = {}
				for _, chest in pairs(cachedChests) do
					if chest:FindFirstChild("TouchInterest") then
						table.insert(valid, chest)
					end
				end

				local torso = getCharacter().LowerTorso
				table.sort(valid, function(a, b)
					return (torso.Position - a.Position).Magnitude <
						   (torso.Position - b.Position).Magnitude
				end)
			...
			local function start()
				...
				task.spawn(function()
					while _G["AutoChestBP"] and player.Character and player.Character.Parent do
						local list = getChests()
						if #list > 0 then
							getCharacter().HumanoidRootPart.CFrame = list[1].CFrame
						end
						task.wait(0.1)
					end
					running = false
...
StopI = L_1_[93]["Main"]:AddToggle({
	["Name"] = "Dừng khi có vật phẩm đặc biệt",
	["Description"] = "Tự dừng farm khi nhặt được Chalice / Fist",
	["Default"] = true,
	["Callback"] = function(Value)
	...
	while wait(Sec) do
		if _G["AutoBerry"] then
			...
			for i = 1, #berryBushes do
				...
				for _, berryType in pairs(bush:GetAttributes()) do
					if not BerryArray or table.find(BerryArray, berryType) then
						_tp(bush.Parent:GetPivot())
						for j = 1, #berryBushes do
							local bush2 = berryBushes[j]
							for _, berry in pairs(bush2:GetChildren()) do
								if not BerryArray or table.find(BerryArray, berry) then
									_tp(berry.WorldPivot)
...
BerryH = L_1_[93]["Main"]:AddToggle({
	["Name"] = "Tự động farm Berry + Đổi server",
	["Description"] = "",
	["Default"] = false,
	["Callback"] = function(Value)
		_G["AutoBerryH"] = Value
					...
					for _, berryType in pairs(bush:GetAttributes()) do
						if not BerryArray or table.find(BerryArray, berryType) then
							...
							for j = 1, #berryBushes do
								local bush2 = berryBushes[j]
								for _, berry in pairs(bush2:GetChildren()) do
									if not BerryArray or table.find(BerryArray, berry.Name) then
										_tp(berry.WorldPivot)
										fireproximityprompt(berry.ProximityPrompt, math.huge)
									end
								end
...
local mobDisplayMap = {
    ...
    ["Isle Outlaw"] = "Cư Dân Ở Đảo Ngoài Vòng Pháp Luật",
    ["Island Boy"] = "Chàng Trai Trên Đảo",
    ["Sun-kissed Warrior"] = "Chiến Binh Mặt Trời",
    ["Isle Champion"] = "Nhà Vô Địch Đảo Tiki"
}

local function addMobDropdown(world, mobList)
    if world then
        local displayNames = {}
        for _, name in ipairs(mobList) do
            table.insert(displayNames, mobDisplayMap[name] or name)
        end
        L_1_[93]["Main"]:AddDropdown({
            ["Name"] = "Chọn Quái Vật",
            ["Default"] = false,
            ["Options"] = displayNames,
            ["Callback"] = function(selectedVn)
                for original, vn in pairs(mobDisplayMap) do
                    if vn == selectedVn then
                        getgenv()["SelectMob"] = original
                        break
                    end
                end
            end
...
addMobDropdown(World1, {
    ...
    "Toga Warrior","Gladiator","Military Soldier",
    "Military Spy","Fishman Warrior","Fishman Commando",
    "God's Guard","Shanda","Royal Squad","Royal Soldier",
    "Galley Pirate","Galley Captain"
})

addMobDropdown(World2, {
    ...
    "Ship Steward","Ship Officer","Arctic Warrior",
    "Snow Lurker","Sea Soldier","Water Fighter"
})

addMobDropdown(World3, {
...
L_1_[32] = {
    ["Kingdom of Rose"] = {
        ...
        ["Mobs"] = {"Raider","Mercenary","Swan Pirate","Factory Staff"}
    };
    ["Green Zone"] = {
        ["CFrame"] = CFrame.new(-2447, 73, -3211),
        ["Mobs"] = {"Marine Lieutenant","Marine Captain"}
    };
    ["Graveyard Island"] = {
    ...
    ["Snow Mountain"] = {
        ...
        ["Mobs"] = {"Snow Trooper","Winter Warrior"}
    };
    ["Hot and Cold (Cold)"] = {
        ["CFrame"] = CFrame.new(-6026, 15, -5062),
        ["Mobs"] = {"Lab Subordinate","Horned Warrior"}
    };
    ["Hot and Cold (Hot)"] = {
    ...
    ["Ice Castle"] = {
        ...
        ["Mobs"] = {"Arctic Warrior","Snow Lurker"}
    };
    ["Forgotten Island"] = {
        ["CFrame"] = CFrame.new(-3043, 238, -10191),
        ["Mobs"] = {"Sea Soldier","Water Fighter"}
    }
}
L_1_[98] = {
    ["Port Town"] = {
        ["CFrame"] = CFrame.new(-290, 44, 5450),
        ["Mobs"] = {"Pirate Millionaire","Pistol Billionaire"}
    };
    ["Hydra Island"] = {
        ["CFrame"] = CFrame.new(5228, 604, 345),
        ["Mobs"] = {"Dragon Crew Warrior","Dragon Crew Archer","Female Islander","Giant Islander","Training Dummy"}
    };
    ["Great Tree"] = {
        ["CFrame"] = CFrame.new(2682, 1682, -7190),
        ["Mobs"] = {"Marine Commodore","Marine Rear Admiral"}
    };
    ["Floating Turtle"] = {
        ["CFrame"] = CFrame.new(-12000, 331, -8500),
        ["Mobs"] = {"Forest Pirate","Mythological Pirate","Jungle Pirate","Musketeer Pirate","Fishman Raider","Fishman Captain"}
    };
    ["Haunted Castle"] = {
        ["CFrame"] = CFrame.new(-9515, 142, 5536),
    ...
    ["Sea of Treats"] = {
        ...
        ["Mobs"] = {"Peanut Scout","Peanut President","Ice Cream Commander","Cookie Crafter","Cake Guard","Baking Staff","Head Baker","Cocoa Warrior","Chocolate Bar Battle","Sweet Thief","Candy Rebel"}
    };
    ["Tiki Outpost"] = {
        ["CFrame"] = CFrame.new(-16200, 90, -17300),
        ["Mobs"] = {"Isle Outlaw","Island Boy","Sun-kissed Warrior","Isle Champion"}
    };
    ["Submerged Island"] = {
        ["CFrame"] = CFrame.new(-3200, -10, -10000),
        ["Mobs"] = {"Reef Bandit","Coral Pirate","Sea Chanter","Ocean Prophet","High Disciple","Grand Devotee"}
    }
}
if World1 then
	L_1_[93]["Main"]:AddDropdown({
		...
		["Options"] = {
			...
			"Hải Quân",          -- Marine
			"Rừng",              -- Jungle
			"Làng Cướp Biển",    -- Pirate Village
			"Sa Mạc",            -- Desert
			"Làng Băng Giá",     -- Frozen Village
			"Pháo Đài Hải Quân", -- Marine Fortress
			"Bầu Trời Thấp",     -- Skylands Lower
			"Nhà Tù",            -- Prison
			"Đấu Trường",        -- Colosseum
			"Làng Dung Nham",    -- Magma Village
			"Thành Phố Dưới Nước", -- Underwater City
			"Bầu Trời Cao"       -- Skylands Upper
		};
		["Callback"] = function(value)
...
if World2 then
	L_1_[93]["Main"]:AddDropdown({
		["Name"] = "Chọn Đảo",
		["Options"] = {
			"Vương Quốc Hoa Hồng", -- Kingdom of Rose
			"Vùng Xanh",            -- Green Zone
			"Đảo Nghĩa Trang",      -- Graveyard Island
			"Núi Tuyết",            -- Snow Mountain
			"Nóng & Lạnh (Lạnh)",  -- Hot and Cold (Cold)
			"Nóng & Lạnh (Nóng)",  -- Hot and Cold (Hot)
			"Con Tàu Bị Nguyền",   -- Cursed Ship
			"Lâu Đài Băng",         -- Ice Castle
			"Đảo Bị Lãng Quên"     -- Forgotten Island
		},
		["Callback"] = function(value)
			_G["SelectIsland"] = value
		end
...
if World3 then
	L_1_[93]["Main"]:AddDropdown({
		["Name"] = "Chọn Đảo",
		["Options"] = {
			"Thị Trấn Cảng",     -- Port Town
			"Đảo Hydra",         -- Hydra Island
			"Cây Khổng Lồ",      -- Great Tree
			"Rùa Nổi",           -- Floating Turtle
			"Lâu Đài Ma Ám",     -- Haunted Castle
			"Biển Kẹo",          -- Sea of Treats
			"Tiền Đồn Tiki",     -- Tiki Outpost
			"Đảo Ngập Nước"      -- Submerged Island
		},
		["Callback"] = function(value)
			_G["SelectIsland"] = value
		end
...
L_1_[93]["Main"]:AddToggle({
    ["Name"] = "Tự Động Farm Tất Cả Đảo", -- Auto Farm All Island
    ["Default"] = false,
    ["Callback"] = function(value)
...
task.spawn(function()
    while task.wait(0.2) do
        ...
        local islandMobs = islandData["Mobs"]
        local mobList = {}

        for index, mobName in ipairs(islandMobs) do
            mobList[mobName] = true
        end

        local foundMob = false

        for _, enemy in pairs(workspace["Enemies"]:GetChildren()) do
            if mobList[enemy["Name"]] and enemy:FindFirstChild("Humanoid") and enemy:FindFirstChild("HumanoidRootPart") and enemy["Humanoid"]["Health"] > 0 then
                foundMob = true
                repeat
                    task.wait()
                    _tp(enemy["HumanoidRootPart"]["CFrame"] * CFrame.new(0,10,0))
                    L_1_[4]["Kill"](enemy, true)
                until not _G["AutoFarmIsland"] or not enemy["Parent"] or enemy["Humanoid"]["Health"] <= 0
            end
        end
...
spawn(function()
	while wait(Sec) do
		pcall(function()
			Process:SetDesc("Tiến Trình Elite :  " .. L_1_[18]["Remotes"]["CommF_"]:InvokeServer("EliteHunter", "Progress"))
		end)
	end
end)
...
spawn(function()
	...
	while wait(1) do
		local statusData = {}
		statusData[3] = ((game:GetService("ReplicatedStorage")):FindFirstChild("Diablo") or (game:GetService("ReplicatedStorage")):FindFirstChild("Deandre") or (game:GetService("ReplicatedStorage")):FindFirstChild("Urban") or (game:GetService("Workspace"))["Enemies"]:FindFirstChild("Diablo") or (game:GetService("Workspace"))["Enemies"]:FindFirstChild("Deandre") or (game:GetService("Workspace"))["Enemies"]:FindFirstChild("Urban")) and "✅️" or "❌️"
		statusData[1] = (game:GetService("ReplicatedStorage"))["Remotes"]["CommF_"]:InvokeServer("EliteHunter", "Progress")
		if statusData[3] ~= lastStatus then
			EliteHunter:SetDesc("Trạng Thái: " .. (statusData[3] .. (" | Đã Giết: " .. statusData[1])))
			lastStatus = statusData[3]
		end
	end
end)
...
-- Spawn loop farm Elite
spawn(function()
	while wait(1) do
		pcall(function()
			if _G["FarmEliteHunt"] then
				local questGui = L_1_[136]["PlayerGui"]["Main"]["Quest"]
				local questTitle = questGui["Container"]["QuestTitle"]["Title"]["Text"]
				if not questGui["Visible"] then
					local result = L_1_[18]["Remotes"]["CommF_"]:InvokeServer("EliteHunter")
					...
					if targetName then
						...
						for _, enemy in pairs(L_1_[18]:GetChildren()) do
						...
						for _, enemy in pairs(Enemies:GetChildren()) do
							if enemy["Name"] == targetName and L_1_[4]["Alive"](enemy) then
								targetEnemy = enemy
								break
							end
						end
						if targetEnemy and targetEnemy:FindFirstChild("HumanoidRootPart") then
							_tp(targetEnemy["HumanoidRootPart"]["CFrame"] * CFrame["new"](0,30,0))
							repeat
								wait()
								L_1_[4]["Kill"](targetEnemy, _G["FarmEliteHunt"])
							until not _G["FarmEliteHunt"] or not targetEnemy["Parent"] or targetEnemy["Humanoid"]["Health"] <= 0 or not questGui["Visible"]
						else
							wait(5)
						end
					else
...
-- Hàm chuyển server khi cần
L_1_[35] = function()
	local httpService = game:GetService("HttpService")
	local teleportService = game:GetService("TeleportService")
	local baseUrl = L_1_[2]({"https://games.roblox", ".com/v1/games/"})
	local placeId = game["PlaceId"]
	local nextCursor = ""
	local teleported = false
	repeat
		local success, data = pcall(function()
			return game:HttpGet(baseUrl .. (placeId .. (L_1_[2]({
				"/servers/Public?sort","Order=Asc&limit=100&","cursor="
			}) .. nextCursor)))
		end)
		if success and data then
			local jsonData = httpService:JSONDecode(data)
			if jsonData["data"] then
				for _, server in pairs(jsonData["data"]) do
					if server["playing"] < server["maxPlayers"] and server["id"] ~= game["JobId"] then
						teleported = true
				...
				nextCursor = jsonData["nextPageCursor"] or ""
			end
		end
	until not nextCursor or teleported
end

-- Spawn loop Auto Farm Elite + Hop
spawn(function()
	while task  do
		pcall(function()
				...
				if not questGui["Visible"] then
					...
					if targetName then
						...
						if targetEnemy and targetEnemy:FindFirstChild("HumanoidRootPart") then
							...
							repeat
								wait()
								L_1_[4]["Kill"](targetEnemy, _G["FarmEliteH"])
							until not _G["FarmEliteH"] or not targetEnemy["Parent"] or targetEnemy["Humanoid"]["Health"] <= 0 or not questGui["Visible"]
						else
							task 
							L_1_[35]()
						end
		...
		pcall(function()
			if _G["AutoRipIngay"] then
				...
				if not GetWP("Dark Dagger") or not GetIn("Valkyrie") and L_465_[2] then
					...
					L_1_[18]["Remotes"]["CommF_"]:InvokeServer(
						"requestEntrance",
						Vector3["new"](
							-5097.93164, 316.447021, -3142.66602,
							-0.405007899, -4.31682743e-08, .914313197,
...
spawn(function()
	while wait(.2) do
		pcall(function()
			...
			if L_479_[1] then
				MobKilled:SetDesc("Đã hạ : " .. 500 - L_479_[1])
			end
		end)
	end
end)
...
CakeQ = L_1_[93]["Main"]:AddToggle({
	["Name"] = "Tự Động Nhận Nhiệm Vụ Bánh ngọt",
	["Description"] = "",
	["Default"] = false,
...
CakeSM = L_1_[93]["Main"]:AddToggle({
	["Name"] = L_1_[2]({
		"Tự Động Triệu Hồi Hoàng Tử Bột",
		""
	}),
	["Description"] = "",
	["Default"] = false,
...
L_1_[93]["Main"]:AddToggle({
	["Name"] = L_1_[2]({
		"Tự Động đánh Vua Bột và chuyển Máy Chủ",
		""
	}),
	["Default"] = false,
...
DouD = L_1_[93]["Main"]:AddToggle({
	["Name"] = L_1_[2]({
		"Tự Động Mở Chip Raid Dough",
		""
	}),
	["Description"] = "Tự Làm Tất Cả",
	["Default"] = false,
...
PhoD = L_1_[93]["Main"]:AddToggle({
	["Name"] = L_1_[2]({
		"Tự Động Mở Chip Raid Phoenix",
		""
	}),
	["Description"] = "Đủ 400 Mastery Trái Phoenix",
	["Default"] = false,
...
spawn(function()
	while wait(.2) do
		pcall(function()
			CheckingBone:SetDesc(" Số Bone : " .. GetM("Bones"))
		end)
	end
end)
...
L_1_[93]["Main"]:AddToggle({
	["Name"] = "Tự Động Triệu Hồi Soul Reaper",
	["Description"] = "";
	["Default"] = false,
...
Pray = L_1_[93]["Main"]:AddToggle({
	["Name"] = "Tự Động Cầu Nguyện Cho Mộ Đá";
	["Description"] = "",
	["Default"] = false;
	...
	pcall(function()
		while wait(1) do
			if workspace["Enemies"]:FindFirstChild("Tyrant of the Skies") then
				TyrantStatus:SetDesc("✅ Đã xuất hiện")
			else
				TyrantStatus:SetDesc("❌ Chưa xuất hiện")
			end
		end
	end)
end)
...
task["spawn"](function()
	...
	while task do
		...
		EyeStatus:SetDesc("Mắt đã kích hoạt: " .. (L_535_[1] .. "/4"))
		...
		if _G["FarmTyrant"] then
			pcall(function()
				...
				for L_539_forvar0, L_540_forvar1 in ipairs(L_538_[5]) do
					...
					for _, L_543_forvar1 in pairs(workspace["Enemies"]:GetChildren()) do
						if not _G["FarmTyrant"] then break end
						if L_543_forvar1 and L_543_forvar1["Name"] == L_540_forvar1
						and L_543_forvar1:FindFirstChild("HumanoidRootPart")
						and L_543_forvar1:FindFirstChild("Humanoid")
						and L_543_forvar1["Humanoid"]["Health"] > 0 then
							repeat
								if not _G["FarmTyrant"] then break end
								L_1_[4]["Kill"](L_543_forvar1, _G["FarmTyrant"])
								wait()
							until not _G["FarmTyrant"] or not L_543_forvar1["Parent"] or L_543_forvar1["Humanoid"]["Health"] <= 0
						end
					end
				end
			end)
...
-- UI hiển thị tiếng Việt, LOGIC VẪN GIỮ Cake / Bone
L_1_[17] = {
	"Đảo Bánh (Cake)",
	"Đảo Xương (Bone)"
}
...
L_1_[74] = L_1_[93]["Main"]:AddDropdown({
	...
	["Callback"] = function(L_604_arg0)
		if L_604_arg0 == "Đảo Bánh (Cake)" then
			SelectIsland = "Cake"
		elseif L_604_arg0 == "Đảo Xương (Bone)" then
			SelectIsland = "Bone"
		end
	end
})
	...
	RunSer.RenderStepped:Connect(function()
		pcall(function()
			if _G["FarmMastery_Dev"] or _G["FarmMastery_G"] or _G["FarmMastery_S"] then
				for _,v in pairs(L_1_[136].PlayerGui.Notifications:GetChildren()) do
		...
		if _G["FarmMastery_Dev"] then
			pcall(function()
				if SelectIsland == "Cake" then
					local mob = GetConnectionEnemies(L_1_[122])
					if mob then
						HealthM = (mob.Humanoid.MaxHealth * 70) / 100
						repeat
							wait()
							MousePos = mob.HumanoidRootPart.Position
							L_1_[4].Mas(mob, true)
						until not _G["FarmMastery_Dev"] or mob.Humanoid.Health <= 0 or not mob.Parent
					else
						_tp(CFrame.new(-1943.6765, 251.5095, -12337.8809))
					end
				elseif SelectIsland == "Bone" then
					local mob = GetConnectionEnemies(L_1_[95])
					if mob then
						HealthM = (mob.Humanoid.MaxHealth * 70) / 100
						repeat
							wait()
